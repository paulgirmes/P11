...
class FoodQuery(forms.Form, ModelForm):
    """
    used to validate food_item queries / display errors /
    using crispy forms (self.helper).
    """
    ...
    def __init__(self, *args, data=None, auto_id=None):
        ...
        self.helper.form_class = "justify-content-center mx-sm-3 form-inline "
        "nav-search"
...
class Signin(UserCreationForm):
    ...
    def save(self, commit=True):
        ...
        except (ValueError, ValidationError, BaseException):
            return self.errors
        # workaround to avoid custom User model creation
        # (username is not necessary per data model design but is required per
        ...
        try:
            ...
            self.add_error(
                "email",
                self.data["email"] +
                " est déjà utilisé par un autre compte, merci d'en utiliser"
                " un autre",
            )
            return self.errors
